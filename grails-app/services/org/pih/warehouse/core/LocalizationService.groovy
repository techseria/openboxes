...
class LocalizationService {
	...
	Locale getCurrentLocale() {
		...
		return (RequestContextHolder.currentRequestAttributes().getSession().user?.locale ?: new Locale(Holders.config.openboxes.locale.defaultLocale))
	}
	...
	Properties getMessagesPropertiesFromClasspath(String messagesPropertiesUrl) {
		Properties properties = new Properties()
		//File messagesPropertiesFile = new ClassPathResource(messagesPropertiesUrl).getInputStream()
        /*ClassPathResource resource = new ClassPathResource(messagesPropertiesUrl)
        File messagesPropertiesFile = resource.getFile()
		log.info "messagesPropertiesFile: ${messagesPropertiesFile}"
		if (messagesPropertiesFile.exists()) {*/
			InputStream messagesPropertiesStream = new ClassPathResource(messagesPropertiesUrl).getInputStream()
