...
//                "category.name": product?.category?.name
        ...
        println("Running liquibase changelog(s) ...")
        ...
        try {
            ...
            FileOpener fileOpener = classLoader.loadClass("util.GrailsFileOpener").getConstructor().newInstance()
            ...
            println "Liquibase running: " + isRunningMigrations
            println "Setting default schema to " + connection.catalog
            println "Product Version: " + database.databaseProductVersion
            println "Database Version: " + database.databaseMajorVersion + "." + database.databaseMinorVersion
        ...
        println("Finished running liquibase changelog(s)!")

		def enableFixtures = Boolean.parseBoolean(Holders.config.openboxes.fixtures.enabled?:"true");
        println("Insert test fixtures?  " + enableFixtures)
		if (enableFixtures) {
            println("Inserting test fixtures ...")
            insertTestFixture()
        }

        //println("Default TimeZone set to " + TimeZone.getDefault().displayName)
    ...
    private def addProductAndInventoryItemIfNotExist(Map<String, Object> inventoryItemInfo) {
        ...
        if (!product) {
            println("creating product ${inventoryItemInfo.product.name}")
