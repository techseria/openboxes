...
class ReceivingPage extends Component {
    ...
    if (this.state.page === 0) {
      ...
      if (!isBinLocationChosen && this.props.hasBinLocationSupport && !(formValues.shipmentStatus === 'RECEIVED')) {
        ...
        this.save({
          ...
          window.location = `${process.env.REACT_APP_BASE_NAME}/stockMovement/show/${requisition}`;
        });
      }
    }
  }
  ...
  */
  ...
  saveValues(formValues) {
    ...
    const url = `${process.env.REACT_APP_BASE_NAME}/api/partialReceiving/${this.props.match.params.shipmentId}?stepNumber=${this.state.page + 1}`;
   ...
   */
  fetchBins() {
    const url = `${process.env.REACT_APP_BASE_NAME}/api/internalLocations/receiving?location.id=${this.state.locationId}&shipmentNumber=${this.state.shipmentNumber}`;
