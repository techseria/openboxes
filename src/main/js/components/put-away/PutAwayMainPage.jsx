...
class PutAwayMainPage extends Component {
  ...
  fetchPutAway() {
    if (this.props.match.params.putAwayId) {
      ...
      const url = `${process.env.REACT_APP_BASE_NAME}/api/putaways/${this.props.match.params.putAwayId}`;
  ...
  savePutAways(putAwayToSave, callback) {
    this.props.showSpinner();
    const url = `${process.env.REACT_APP_BASE_NAME}/api/putaways?location.id=${this.props.location.id}`;
   ...
   */
  firstPage() {
    this.props.history.push(`${process.env.REACT_APP_BASE_NAME}/putAway/create`);
