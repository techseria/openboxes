...
class PutAwaySecondPage extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    showLocationChangedAlert(
      ...
      () => { window.location = `${process.env.REACT_APP_BASE_NAME}/order/list?orderTypeCode=TRANSFER_ORDER&status=PENDING`; },
    );
  ...
  fetchBins() {
    ...
    const url = `${process.env.REACT_APP_BASE_NAME}/api/internalLocations?location.id=${this.props.location.id}&locationTypeCode=BIN_LOCATION`;
  ...
  deleteItem(itemIndex) {
    this.props.showSpinner();
    const url = `${process.env.REACT_APP_BASE_NAME}/api/putawayItems/${_.get(this.props.putAway.putawayItems, `[${itemIndex}].id`)}`;
  ...
  generatePutAwayList() {
    this.props.showSpinner();
    const url = `${process.env.REACT_APP_BASE_NAME}/putAway/generatePdf/ff80818164ae89800164affcfe6e0001`;
  ...
  fetchItems(sortBy) {
    const url = `${process.env.REACT_APP_BASE_NAME}/api/putaways/${this.props.putAway.id}?sortBy=${sortBy}`;
    return apiClient.get(url)
      .then((response) => {
