...
class PutAwayPage extends Component {
  ...
  fetchPutAwayCandidates(locationId) {
    ...
    const url = `${process.env.REACT_APP_BASE_NAME}/api/putaways?location.id=${locationId}`;
  ...
  createPutAway() {
    this.props.showSpinner();
    const url = `${process.env.REACT_APP_BASE_NAME}/api/putaways?location.id=${this.props.locationId}`;
    ...
    return apiClient.post(url, flattenRequest(payload))
      .then((response) => {
        ...
        this.props.history.push(`${process.env.REACT_APP_BASE_NAME}/putAway/create/${putAway.id}`);
