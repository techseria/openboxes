...
class CreateStockMovement extends Component {
  ...
  fetchStockLists(origin, destination, clearStocklist) {
    ...
    const url = `${process.env.REACT_APP_BASE_NAME}/api/stocklists?origin.id=${origin.id}&destination.id=${destination.id}`;
    console.log(url);
  ...
  saveStockMovement(values) {
    if (values.origin && values.destination && values.requestedBy &&
      ...
      if (values.stockMovementId) {
        stockMovementUrl = `${process.env.REACT_APP_BASE_NAME}/api/stockMovements/${values.stockMovementId}/updateRequisition`;
      } else {
        stockMovementUrl = `${process.env.REACT_APP_BASE_NAME}/api/stockMovements`;
      }
      console.log(values.description);
      console.log(values.origin.id);
      console.log(values.dateRequested);
      console.log(values.requestedBy);
      const payload = {
        name: '',
      ...
      apiClient.post(stockMovementUrl, payload)
        .then((response) => {
          if (response.data) {
            ...
            this.props.history.push(`${process.env.REACT_APP_BASE_NAME}/stockMovement/create/${resp.id}`);
            this.props.onSubmit({
              ...values,
