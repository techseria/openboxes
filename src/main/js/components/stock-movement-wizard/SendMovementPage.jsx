...
class SendMovementPage extends Component {
   ...
   */
  fetchShipmentTypes() {
    const url = `${process.env.REACT_APP_BASE_NAME}/api/generic/shipmentType`;
  ...
  fetchStockMovementData() {
    const url = `${process.env.REACT_APP_BASE_NAME}/api/stockMovements/${this.state.values.stockMovementId}?stepNumber=6`;
   ...
   */
  sendFile(file) {
    const url = `${process.env.REACT_APP_BASE_NAME}/stockMovement/uploadDocument/${this.state.values.stockMovementId}`;
  ...
  saveShipment(payload) {
    const url = `${process.env.REACT_APP_BASE_NAME}/api/stockMovements/${this.state.values.stockMovementId}/updateShipment`;
        ...
        .then(() => {
          this.stateTransitionToIssued()
            .then(() => {
              ...
              window.location = `${process.env.REACT_APP_BASE_NAME}/stockMovement/show/${this.state.values.stockMovementId}`;
            })
            .catch(() => this.props.hideSpinner());
        })
        .catch(() => this.props.hideSpinner());
  ...
  saveAndExit(values) {
    ...
    if (_.isEmpty(errors)) {
      this.saveValues(values)
        .then(() => {
          window.location = `${process.env.REACT_APP_BASE_NAME}/stockMovement/show/${values.stockMovementId}`;
        });
    } else {
      confirmAlert({
        ...
        buttons: [
          {
            ...
            onClick: () => { window.location = `${process.env.REACT_APP_BASE_NAME}/stockMovement/show/${values.stockMovementId}`; },
          },
          {
                ...
                <div className="print-buttons-container col-md-3 flex-grow-1">
                  {this.state.documents.length && _.map(this.state.documents, (document, idx) => {
                    ...
                    if (document.contentType.includes('pdf')) {
                        this.logopath = '/static/images/icons/pdf.png';
                    } else if (document.contentType.includes('ms-excel')) {
                        this.logopath = '/static/images/icons/silk/page_white_excel.png';
                    } else if (document.contentType.includes('html')) {
                      this.logopath = '/static/images/icons/silk/html_go.png';
                    }
                    return (
                      <DocumentButton
                        link={document.uri}
